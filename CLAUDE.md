...
- **Never fabricate quotes, actions, or routines attributed to real people.** When writing about a named subject (Roberto, Avenamar, a café owner), source the material first: ask the user for written quotes, transcribe the video with `whisper-cli` (model at `~/.cache/hyperframes/whisper/models/ggml-large-v3.bin`, audio extracted via `ffmpeg -vn -ar 16000 -ac 1 -c:a pcm_s16le`), and save the transcript under `docs/journal-source/` for future reference. `<PullQuote attribution="Real Name">` must contain text that maps to something the subject actually said. Industry-knowledge claims that aren't attributed to anyone are fine; anything tagged with a name is not.
