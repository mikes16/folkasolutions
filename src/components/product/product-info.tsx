...
export function ProductInfo({ product, selectedVariantIndex: externalIndex, onVariantChange }: ProductInfoProps) {
  ...
  return (
    ...
    <div className="flex flex-col gap-4 md:gap-6">
                      ...
                      onClick={() => {
                        ...
                        if (idx !== -1) {
                          ...
                          posthog.capture("variant_selected", {
                            ...
                            option_value: value,
                            variant_id: product.variants[idx]?.id,
                            price: product.variants[idx]?.price.amount,
                            currency: product.variants[idx]?.price.currencyCode,
                          });
                        }
                      }}
                      className={
      ...
      {/* WhatsApp: reserved for high-ticket equipment (see whatsapp-gate.ts) */}
      {variant && shouldShowWhatsappCta(variant.price) && (
        <a
          href={whatsappLink(
          ...
          onClick={() => {
            posthog.capture("whatsapp_inquiry_clicked", {
              product_title: product.title,
              product_id: product.id,
      ...
      <div className="flex items-center gap-3 py-3">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-base font-bold">
            ...
            {shouldShowTaxNote(country) && (
              <span className="text-[10px] text-muted ml-1">{tp("taxNoteShort")}</span>
            )}
          </p>
        </div>
        <Button
